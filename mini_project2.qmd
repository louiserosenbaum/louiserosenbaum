...
success_rate_for_fg |>
  for(i in 1:1000) {
    sample (fg_summary, replace = TRUE) 
    #fit model 
    lm(succ ~ distance, data = Dist) |>
      
    calculate distance 
    store dist
  }

m1 <- lm(SuccessRate ~ Dist, data = fg_summary)
summary(m1)
int <- m1$coefficients[1]
slope <- m1$coefficients[2]
int
slope
dist_belowhalf <- (.50 - int) / slope
dist_belowhalf

ggplot() +
  geom_histogram(aes(1000 distance)) +
  quantile(dist, .025, .0975) 
```
